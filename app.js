...
let ALL = [];
let list = [];            // 当前练习列表
let pos = 0;              // 当前题在 list 中的下标
let mode = "seq";
let reviewMode = false;
let sel = [];             // 当前选择
let sessionAnswered = new Set();   // 本次会话已答 id
let correctCount = 0;
...
function init() {
  if (!window.QUESTIONS || !window.QUESTIONS.questions) {
    document.getElementById("ctrl").innerHTML =
      '<div class="empty">题库未加载：请确认 questions.js 与本网页在同一文件夹，且用 &lt;script&gt; 引入。</div>';
    return;
  }
...
function startPractice(asWrong) {
  ...
  let pool = ALL.filter((q) => t === "all" || q.type === t);
...
function render() {
  ...
  const id = hashId(q);
  sel = [];
  $("qType").className = "qtype qtype-" + q.type;
  $("qType").innerHTML = TYPE_LABEL[q.type] + (q.review ? '<span class="tag-review">待核对</span>' : "");
  ...
  if (q.type === "fill") {
    ...
    inp.type = "text"; inp.className = "fillinput"; inp.placeholder = "输入答案后点“提交 / 看答案”";
...
function submit() {
  ...
  let userAns;
  if (q.type === "fill") userAns = $("fillInput").value;
  else userAns = sel.slice().sort().join("");

  let ok;
  if (q.type === "fill") ok = fillCorrect(userAns, q.answer || "");
...
function reveal(q, id, justAnswered, ok) {
  ...
  if (q.type === "multiple" || q.type === "single" || q.type === "judge") {
    ...
    document.querySelectorAll("#qBody .opt").forEach((o) => {
      ...
      const chosen = (q.type === "multiple" ? sel.includes(k) : sel[0] === k);
...
function answerText(q) {
  if (q.type === "fill") return q.answer || "（暂无标准答案，请到题库管理补充）";
